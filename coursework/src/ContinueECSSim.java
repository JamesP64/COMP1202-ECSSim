...
public class ContinueECSSim {
  ...
  public ContinueECSSim() throws IOException, InterruptedException {

    //The aim of this constructor is to rebuild the simulation based on the saved data

    // Take data from save.txt
    BufferedReader saveReader = new BufferedReader(new FileReader("save.txt"));
    university = new University((int) Float.parseFloat(saveReader.readLine()));
    estate = university.getEstate();
    HR = university.getHR();
    university.setReputation(Integer.parseInt(saveReader.readLine()));
    numOfHalls = Integer.parseInt(saveReader.readLine());
    numOfLabs = Integer.parseInt(saveReader.readLine());
    numOfTheatres = Integer.parseInt(saveReader.readLine());
    year = Integer.parseInt(saveReader.readLine());
    staffMarket = new ArrayList<>();

    // Make list of existing facilities

    // Get length of facilities file
    BufferedReader reader = new BufferedReader(new FileReader("Facilities.txt"));
    int lines = 0;
    while (reader.readLine() != null) lines++;
    reader.close();

    // Move data from txt file to university
    for (int i = 0; i < (lines / 3); i++) {
      BufferedReader facilitiesReader = new BufferedReader(new FileReader("Facilities.txt"));
      university.build(facilitiesReader.readLine(), facilitiesReader.readLine());
      estate.getFacilityList().get(i).setLevel(Integer.parseInt(facilitiesReader.readLine()));
    }

    // Create Staff, and populate staffSalary HashMap

    // Get length of staffSalary file
    BufferedReader reader2 = new BufferedReader(new FileReader("staffSalary.txt"));
    int lines2 = 0;
    while (reader2.readLine() != null) lines++;
    reader.close();

    // Move data from txt file to create staff and populate HashMap
    for (int i = 0; i < (lines / 4); i++) {
      BufferedReader staffSalaryReader = new BufferedReader(new FileReader("staffSalary.txt"));
      Staff staff =
          new Staff(
              staffSalaryReader.readLine(),
              Integer.parseInt(staffSalaryReader.readLine()),
              university);
      staff.setYearsOfTeaching(Integer.parseInt(staffSalaryReader.readLine()));
      HR.addStaff(staff, Float.parseFloat(staffSalaryReader.readLine()));
    }

    // Get length of staffMarket file
    BufferedReader reader3 = new BufferedReader(new FileReader("staffMarket.txt"));
    int lines3 = 0;
    while (reader2.readLine() != null) lines++;
    reader.close();

    // Populate the Staff market with staff that havent been hired yet
    for (int i = 0; i < (lines / 2); i++) {
      BufferedReader staffMarketReader = new BufferedReader(new FileReader("staffMarket.txt"));
      Staff staff =
          new Staff(
              staffMarketReader.readLine(),
              Integer.parseInt(staffMarketReader.readLine()),
              university);
  ...
  public void writeFile() throws IOException {
    ...
    // This one is set to false, so it can overwrite previous saves
    ...
    for (Map.Entry<Staff, Float> entry : HR.getStaffSalary().entrySet()) {
      ...
      staffFileWriter.newLine();
      // Staff stamina
      staffFileWriter.write(String.valueOf(entry.getKey().getStamina()));
      staffFileWriter.newLine();
